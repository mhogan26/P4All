...
// CACHE LOOKUP ACTIONS/TABLES -->
action check_cache_exist_act(index) {
    modify_field (meta.cache_exist, 1);
    modify_field (meta.cache_index, index);
}
...
action check_cache_valid_act() {
    register_read(meta.cache_valid, cache_valid_reg, meta.cache_index);
}
...
action set_cache_valid_act() {
    register_write(cache_valid_reg, meta.cache_index, 1);
}
